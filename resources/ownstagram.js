var ownStaGram = {
	"url": "app.php",
	"startWithImage": "noimage",
	"type": "website",
	"md5salt": "a4916ab01df010a042c612eb057b4ac23e79530d555354c4a92e1b24301b964f0f7ecd66143c4093ea1470efcfa33042",
	
	"setAppUrl": function() {
		this.url = $('#login_server').val();
		if(this.url.substring(this.url.length-1)!="/") this.url += "/";
		this.url += "app.php";
	},
	
	"startComment": function(form, id) {
		
		if($(form).find('#comment_text').val()=='') return this.error('no comment given!');
				...
				"success": function(data) {
					if(data.result==1) {
						if(ownStaGram.type=="app") {
							...
							window.location = 'index.php?action=detail&id='+id;
						}
					}
				}
		});
	...
	"startRegister": function(form) {
		...
		$.ajax({
				...
				"success": function(data) {
					if(data.result==1) {
						window.location = 'index.php?action=thanks&res='+data.result;
					...
					if(data.result==2) {
						...
						if(ownStaGram.type=="app") {
							...
							$('#screen_settings').fadeOut( function() {
									...
									$('#used_server').html( $('#login_server').val() );
							} );
	...
	"saveSettings": function() {
		...
		var data = {'action': "sitesettings"};
		$('.form-signin').find('.settingform').each( function() {
...
function openIframe(id) {
	...
	if(action=="detail") {
		html += '<iframe src="index.php?x=1&O='+id+'" style="width:'+$(window).width()+'px;height:'+($(document).height()-h1)+'px;" border=0 frameborder=0>';
	} else {
	...
	history.pushState({ }, "Details", "index.php?O="+id);
